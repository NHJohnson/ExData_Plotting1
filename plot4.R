...
png(file='plot4.png')
par(mfrow=c(2,2))
plot(hpcdata$timestamp,hpcdata$Global_active_power,type='n',xlab='',ylab='Global Active Power')
lines(hpcdata$timestamp, hpcdata$Global_active_power, type='l')
plot(hpcdata$timestamp,hpcdata$Voltage,type='n',xlab='datetime',ylab='Voltage')
lines(hpcdata$timestamp, hpcdata$Voltage, type='l', col='black')
...
legend("topright",col=c('black','red','blue'),lwd=1,lty=c(1,1,1),legend=names(hpcdata)[7:9],bty='n')
plot(hpcdata$timestamp,hpcdata$Global_reactive_power,type='n',xlab='datetime',ylab='Global_reactive_power')
lines(hpcdata$timestamp, hpcdata$Global_reactive_power, type='l', col='black')
dev.off()